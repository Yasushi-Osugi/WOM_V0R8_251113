...
# ... (V0R7の関数群: build_tree_from_csvなどそのまま)
# get_nodes追加 (ツリー走査)
...
def _create_tree_set_attribute_from_rows(rows: list[dict]) -> tuple[dict[str, Node], str]:
    """
    create_tree_set_attribute(file_name) の rows 版。
    rows は read_tree_file(file_name) 相当の [{'Parent_node':..., 'Child_node':..., 'Product_name':...}, ...]
    を想定。
    """
    nodes: dict[str, Node] = {}
    root_node_name = ""
...
def set_attribute_from_csv(*args, **kwargs):
    """
    後方互換のためのプレースホルダ。
    例：コストCSVを別途読んで node_dict の SKU に反映する場合に使用。
    """
    ...
    # 修正: positional arg の 0番目 (root) を返す
    if args:
        return args[0]
    return None
